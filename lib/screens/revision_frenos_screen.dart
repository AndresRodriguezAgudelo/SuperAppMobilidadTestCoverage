...
class RevisionFrenosScreen extends StatefulWidget {
  ...
  const RevisionFrenosScreen({
    super.key,
    required this.alertId,
  });

  @override
  State<RevisionFrenosScreen> createState() => _RevisionFrenosScreenState();
}

class _RevisionFrenosScreenState extends State<RevisionFrenosScreen> {
  late String nombreVencimiento = 'Revisión de frenos';
      ...
      if (mounted) {
        if (success) {
          ...
          showConfirmationModal(
            ...
            label: 'Revisión de frenos actualizada correctamente',
          );
          ...
          try {
            ...
            if (homeBloc.cars.isNotEmpty) {
              ...
              if (selectedCar == null || !selectedCar.containsKey('id') || !selectedCar.containsKey('licensePlate')) {
                print('\n⚠️ REVISION_FRENOS: No se pudo obtener un vehículo válido');
                return;
              }
              
              print('\n🚗 REVISION_FRENOS: Usando vehículo seleccionado: ${selectedCar["licensePlate"]}');
              vehicleId = selectedCar["id"];

              print(
                  '\n🔄 REVISION_FRENOS_SCREEN: Actualizando alertas para vehículo: ${selectedCar["licensePlate"]} (ID: $vehicleId)');
