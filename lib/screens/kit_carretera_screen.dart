...
class _KitCarreteraScreenState extends State<KitCarreteraScreen> {
  ...
  Future<void> _loadAlertDetails() async {
    ...
    try {
      ...
      print(
          '\n🔥 Kit de carretera_SCREEN: Iniciando carga de alerta ID: ${widget.alertId}');
      ...
      if (_alertsBloc.alertData != null) {
        print('\n🔥 Kit de carretera_SCREEN: Datos cargados correctamente');
        print('Claves disponibles: ${_alertsBloc.alertData!.keys.toList()}');
        _alertsBloc.alertData!.forEach((key, value) {
          print('$key: $value');
        });
      } else {
        print('\n🔥 Kit de carretera_SCREEN: No se pudieron cargar los datos');
      ...
      print('\n🔥 Kit de carretera_SCREEN: Error al cargar los detalles: $e');
      if (mounted) {
        setState(() {
  ...
  Future<void> _saveAlert() async {
    ...
    try {
      ...
      print('\n💾 Kit de carretera_SCREEN: Guardando alerta con datos:');
      ...
      if (success) {
        print(
            '\n🔄 Kit de carretera_SCREEN: Recargando datos de la alerta después de guardar');
      ...
      if (mounted) {
        if (success) {
          ...
          showConfirmationModal(
            ...
            label: 'Kit de carretera actualizado correctamente',
          );
          ...
          try {
            ...
            if (homeBloc.cars.isNotEmpty) {
              ...
              if (selectedCar == null || !selectedCar.containsKey('id') || !selectedCar.containsKey('licensePlate')) {
                print('\n⚠️ KIT_CARRETERA: No se pudo obtener un vehículo válido');
                return;
              }
              
              print('\n🚗 KIT_CARRETERA: Usando vehículo seleccionado: ${selectedCar["licensePlate"]}');
              vehicleId = selectedCar["id"];

              print(
                  '\n🔄 Kit de carretera_SCREEN: Actualizando alertas para vehículo: ${selectedCar["licensePlate"]} (ID: $vehicleId)');
            ...
            appBar: PreferredSize(
              ...
              child: Consumer<SpecialAlertsBloc>(
                builder: (context, alertsBloc, _) {
                  // Obtener el estado del Kit de carretera directamente de la propiedad 'estado'
                  String? status;
                  if (alertsBloc.alertData != null) {
                    // Imprimir todas las claves disponibles para depuración
                    print(
                        '\n🔥 Kit de carretera_SCREEN: Claves disponibles en alertData: ${alertsBloc.alertData!.keys.toList()}');
                    ...
                    if (alertsBloc.alertData!.containsKey('estado')) {
                      ...
                      print(
                          '\n🔥 Kit de carretera_SCREEN: Estado encontrado en "estado": $status');
                    }

                    // Si aún no encontramos el estado, intentar inferirlo de otros campos
                    if (status == null) {
                      print(
                          '\n🔥 Kit de carretera_SCREEN: No se encontró estado explícito, intentando inferirlo...');

                      // Imprimir todos los valores para depuración
                      alertsBloc.alertData!.forEach((key, value) {
                        print('\n🔥 Kit de carretera_SCREEN: $key: $value');
                      });
                      ...
                      if (alertsBloc.alertData!.containsKey('expirationDate') &&
                        ...
                        try {
                          ...
                          print(
                              '\n🔥 Kit de carretera_SCREEN: Fecha de vencimiento: $expirationDate, diferencia en días: $difference');
                          ...
                          print(
                              '\n🔥 Kit de carretera_SCREEN: Estado inferido a partir de la fecha: $status');
                        } catch (e) {
                          print(
                              '\n🔥 Kit de carretera_SCREEN: Error al inferir estado: $e');
                        }
                      } else {
                        print(
                            '\n🔥 Kit de carretera_SCREEN: No se encontró fecha de vencimiento para inferir estado');
                      }
                    }
                    ...
                    if (status == null) {
                      if (fechaVencimiento != null) {
                        ...
                        print(
                            '\n🔥 Kit de carretera_SCREEN: Estado inferido a partir de la fecha local: $status');
                      } else {
                        // Valor por defecto si no hay información
                        status = 'Configurar';
                        print(
                            '\n🔥 Kit de carretera_SCREEN: Usando estado por defecto: $status');
                      }
                    }
                  } else {
                    print(
                        '\n🔥 Kit de carretera_SCREEN: No hay datos de alerta disponibles');
                    if (fechaVencimiento != null) {
                      ...
                      print(
                          '\n🔥 Kit de carretera_SCREEN: Estado inferido a partir de la fecha local (sin datos): $status');
                    }
                  }
